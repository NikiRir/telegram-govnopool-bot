...
function handleDriverInput(chatId, userId, text, session) {
    ...
    switch (step) {
        case 'from':
            ...
            // УБРАЛИ СООБЩЕНИЕ "ИСПРАВЛЕНО" - ТИХАЯ АВТОКОРРЕКЦИЯ
        ...
        case 'contact':
            ...
            // УБРАЛИ СООБЩЕНИЕ ОБ АВТОКОРРЕКЦИИ - ТИХАЯ КОРРЕКЦИЯ
...
function showDriverPreview(chatId, userId, data, userInfo) {
    ...
    try {
        const preview = createDriverForm(data, userInfo);
...
function showPassengerPreview(chatId, userId, data, userInfo) {
    ...
    try {
        const preview = createPassengerForm(data, userInfo);
...
function publishDriverAd(chatId, userId) {
    ...
    const message = createDriverForm(session.data, session.userInfo);
...
function publishPassengerAd(chatId, userId) {
    ...
    const message = createPassengerForm(session.data, session.userInfo);
